...
class Model(nn.Module):
    def __init__(self, num_levels, num_flows, conv_type, flow_type, num_blocks, hidden_channels, image_size=32,
                 in_channels=3, matrix_exp="default"):
        ...
        for i in range(num_levels):
            ...
            flows = [Flow(conv_type, flow_type, num_blocks, in_channels, hidden_channels, image_size, matrix_exp) for _ in
                     range(num_flows[i])]
            if i < num_levels - 1:
...
class Flow(Sequential):
    ...
    def __init__(self, conv_type, flow_type, num_blocks, in_channels, hidden_channels, image_size, matrix_exp):
        super(Flow, self).__init__()
        self.add_module('actnorm', ActNorm(in_channels, image_size))
        self.add_module('conv1x1', Conv1x1(in_channels, conv_type, matrix_exp))
        self.add_module('couplinglayer', CouplingLayer(flow_type, num_blocks, in_channels, hidden_channels, matrix_exp))
