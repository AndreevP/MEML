...
class Conv1x1(nn.Module):
    ...
    def __init__(self, in_channels, conv_type, matrix_exp="default"):
...
class CouplingLayer(nn.Module):
    ...
    def __init__(self, flow_type, num_blocks, in_channels, hidden_channels, matrix_exp="default"):
        ...
        if self.flow_type == 'additive':
            ...
            if not reverse:
                if self.x2_channels <= self.max_out:
                    ...
                    x2 = torch.matmul(expm(weight, self.matrix_exp), x2).permute(0, 3, 4, 1, 2).squeeze(2) + shift
                    out = torch.cat([x1, x2], dim=1)
                    ...
                    weight = torch.eye(self.x2_channels, device=x.device) + torch.matmul(
                        torch.matmul(weight2, series(weight3, self.matrix_exp)), weight1)
                    x2 = x2.unsqueeze(2).permute(0, 3, 4, 1, 2)
                ...
                if self.x2_channels <= self.max_out:
                    ...
                    x2 = torch.matmul(expm(-weight, self.matrix_exp), x2).permute(0, 3, 4, 1, 2).squeeze(2)
                    out = torch.cat([x1, x2], dim=1)
